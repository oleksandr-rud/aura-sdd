...
export async function demonstrateChatUsage() {
  ...
  try {
    ...
    console.log(
      `✅ Message sent. AI response: ${message1.aiMessage?.content?.substring(0, 100)}...`
    )
    ...
    console.log(
      `✅ Follow-up sent. AI response: ${message2.aiMessage?.content?.substring(0, 100)}...`
    )
    ...
    console.log(
      `✅ Session stats: ${stats.messageCount} messages, ${stats.totalTokens} total tokens`
    )
...
export async function demonstrateErrorHandling() {
  ...
  if (invalidSessionResult.isErr()) {
    console.log(
      "✅ Properly handled invalid session error:",
      invalidSessionResult.unwrapErr().message
    )
  }
