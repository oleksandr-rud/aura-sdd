...
export class JWTTokenService implements TokenService {
  ...
  constructor() {
    ...
    if (process.env.NODE_ENV !== 'production') {
      if (this.accessSecret.includes('default')) {
        console.warn('⚠️  Using default JWT secrets. Set JWT_SECRET and JWT_REFRESH_SECRET environment variables in production.')
      }
    }
  }
  ...
  private async generateToken(
    ...
    try {
      ...
      throw new Error(`Failed to generate ${type} token: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
